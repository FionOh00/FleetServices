...
export class GridComponent implements OnInit {
  ...
  buildContextMenuItems(event: any) {
    ...
    setTimeout(() => {
      ...
      console.log(body);
      this.http
        .post<any[]>(url, body) //.subscribe(data => {
        // this.postId = data.id;
        // });
        .subscribe({
          next: (data) => {
  ...
  loadData() {
    ...
    setTimeout(() => {
      ...
      this.http
        .get<any[]>('https://sc.aos.biz/FleetServicesWebAPI/FleetServices/FleetServices')
  ...
  isReverseImage(val: string): boolean {
    if (!val || val == '' || val.length < 2) return false;
    else if (val.endsWith('Y')) return true;
    return false;
  }

  getSeverity(colorCode: string): any {
    if (!colorCode || colorCode == '' || colorCode.length < 2) return '';
  ...
  showHideColumn(columnIndex: number): any {
    var result;
    if (columnIndex < 25) result = this.isPageToggled;
    else result = !this.isPageToggled;
    return result;
  }
